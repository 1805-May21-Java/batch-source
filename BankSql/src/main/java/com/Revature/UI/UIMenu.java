...
public class UIMenu implements Runnable {
	...
	private ProfileAccountDaoImpl linkDao;
	...
	public UIMenu() {
		...
		linkDao = new ProfileAccountDaoImpl();
	}
	...
	private void checkBalance() {
		...
		try {
			...
			List<BankAccount> accList = linkDao.getAllAccounts(currentUser.getUsername());
			if (accList.size() == 0) {
	...
	private void addUserToAccount() {
		...
		try {
			...
			if (joiner == null) { // Validates login of joiner
				...
				bankDao.addProfileToBankAccount(joiner, acc);
