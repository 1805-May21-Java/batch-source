var acBbOrbitalPeriodAJAX = 365.25;
var acBbOrbitalPeriodFetch = 365.25;
...
window.onload = function() {
    ...
    printSpans(); // 6
    showCurrentTime(); // 7
    getacBbOrbitalPeriodAJAX();
    getacBbOrbitalPeriodFetch();
    showNonEarthTimes(); // 8
    bgColorOnTitleClick(); // 9
    calcListeners(); // 10
    walkTheDom(document, testWalk); // 11
}
...
function allFormInputValid(input) {
    ...
    // firstname at least length 2
    if (input.firstname.length < 2) {
        console.log("firstname too short")
        return false;
    }
    // lastname at least length 2
    if (input.lastname.length < 2) {
        console.log("lastname too short");
        return false;
    }
    ...
    // phone number matches 10 digits regex
