...
private static ManageMenu manageMenu=null;
	...
	public void switcher() {
		...
		case 1:
			clear();
			deposit_withdraw((ArrayList<BankAccount>) account.getBankAccounts(), 'd');
			break;
		case 2:
			clear();
			deposit_withdraw((ArrayList<BankAccount>) account.getBankAccounts(), 'w');
			break;
		case 3:
			clear();
			...
			if(account.getBankAccounts().isEmpty()) {
				...
				if(account.getBankAccounts().size()>1)
					index=goThroughBankAccounts((ArrayList<BankAccount>) account.getBankAccounts());
				else
					index=1;
	...
	private void deposit_withdraw(ArrayList<BankAccount> bankAccounts, char which) {
		...
		bankAccounts=(ArrayList<BankAccount>) account.getBankAccounts();
		if(bankAccounts.isEmpty()) {
			...
			while(amount<0) {
				...
				if(amount>0) {
					boolean allGood;
					if(which=='d') {
						allGood=current.depsoit(current, amount);
						if(allGood)
						...
						allGood=current.withdraw(current, amount);
						if(allGood)
