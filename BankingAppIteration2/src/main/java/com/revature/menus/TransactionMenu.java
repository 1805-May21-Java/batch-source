...
public class TransactionMenu implements Menu{
	...
	public boolean navigate() {
		...
		if(this.currentAccount.getJointUsers().size()>1) {
			System.out.print("This is a joint account between: ");
			for(Integer i : this.currentAccount.getJointUsers()) {
				System.out.print(udi.getUserById(i).getUsername() + " ");
			}
			System.out.print("\n");
		}
		...
		do {
			try {
				...
				if(amount < 0) {
					...
					this.tdi.createTransaction(new Transaction( acct,
