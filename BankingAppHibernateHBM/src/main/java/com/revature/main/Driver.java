	...
	public static void main(String[] args) {
		...
		while(currentAccount.isUserLoggedOn()) {
			...
			if(transactLine.equalsIgnoreCase("A")) {
				...
				//ad1.depositFunds(currentAccount.getId(), accountType, deposit);
				...
				//Updates database Account
				//ad1.withDrawFunds(currentAccount.getId(), accountType, withdrawal);
				...
				ad1.deleteAccountById(currentAccount.getId());
		...
		//Session s and Scanner userInput are closed to prevent resource leaks.
		userInput.close();
		s.close();
	}
