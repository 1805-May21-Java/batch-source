package one;

public class Fish extends Animal {

	// Because Fish extends Animal, it must have properties numOfLegs and skinType (Inheritance)
	int numOfLegs = 0;
	String skinType = "scales";
	
	// Because Fish extends Animal, Bird must contain a move() method (Inheritance)
	// Overriding the move method below with a command different from its parent class
	// is an example of Polymorphism 
	public void move() {
		System.out.println("The Fish swims away!");
	}

	public Fish() {
		super();
		// TODO Auto-generated constructor stub
	}

	public int getNumOfLegs() {
		return numOfLegs;
	}

	public void setNumOfLegs(int numOfLegs) {
		this.numOfLegs = numOfLegs;
	}
}
