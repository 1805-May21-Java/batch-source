...
if(sessionStorage.getItem("employee")){
    ...
    getOldEmployeeReimbursements(JSON.parse(sessionStorage.getItem("reimbursements")))
}else{
...
function getNewEmployeeReimbursements(xhr){
    ...
    sessionStorage.setItem("reimbursements",JSON.stringify(response));
    if(response){
	    ...
	    for(reimbursement of response){
	        ...
	        child2.innerHTML = reimbursement.reimbursementAmount;
